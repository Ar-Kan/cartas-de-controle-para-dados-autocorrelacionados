...
coeficientes <- function(phi) {
  ...
  list(alpha = 0, nu = 20, phi = phi, d = 0, nu = 20)
}

barma.sim <- function(n, phi, seed = NULL, y.start = NULL){
  BARFIMA.sim(
    n = n,
...
teste_montecarlo <- cache_dados(
  ...
  function() {
    gerador_monte_carlo(
      parametros = list(
        nH0 = c(rep(25, 20), rep(50, 20), rep(100, 20), rep(200, 20))
      ),
...
```{r, class.source = 'fold-show'}
ewma_qcc <- function(amostra_inicial, lambda, amostra_subsequente, nsigmas = 1.96) {
  ew <- ewma(
    amostra_inicial,
    lambda = lambda,
    nsigmas = nsigmas,
...
ggplotly(
  ewma_monte_carlo %>%
    ...
    geom_point(data = . %>% filter(fora_de_controle),
               aes(x = observacao, y = ewma, color = "Fora de controle"), size = 1, shape = 4) +
...
ggplotly(
  ewma_monte_carlo_resumo %>%
      ggplot(aes(x = h1_phi, y = mean, color = lambda)) +
      geom_line() +
      geom_point() +
      labs(
        x = "Valores de Φ",
...
ggplotly(
  ewma_monte_carlo %>%
    ggplot(aes(x = factor(h1_phi), y = fracao_fora_de_controle, fill = lambda)) +
    geom_boxplot() +
    labs(
      x = "Valores de Φ",
      y = "Fração de pontos fora de controle",
...
Vamos ver se, alterando também o valor de $\alpha$, obtemos um melhor resultado.

```{r}
ewma_geral_monte_carlo <- cache_dados(
  "simulacao-ewma-geral",
  function() {
    gerador_monte_carlo(
      parametros = expand.grid(
        lambda = seq(0.1, 0.9, by = 0.2),
        alfa = seq(0.02, 0.1, by = 0.02)
      )
    ) %>%
      mutate(
        qcc = list(ewma_qcc(h0_residuos, lambda, h1_residuos, qnorm(1 - alfa))),
        ewma = list(qcc$ewma),
        LI = list(qcc$LI),
        LS = list(qcc$LS),
        fora_de_controle = list(qcc$fora_de_controle),
...
```{r}
ewma_geral_monte_carlo$`(λ;α)` <- as.factor(paste(ewma_geral_monte_carlo$lambda, ewma_geral_monte_carlo$alfa, sep = ";"))

ewma_geral_monte_carlo_resumo <- ewma_geral_monte_carlo %>%
  group_by(`(λ;α)`, lambda, alfa, h1_phi) %>%
  summarise(
    mean = mean(fracao_fora_de_controle),
    min = min(fracao_fora_de_controle),
    max = max(fracao_fora_de_controle),
...
datatable(
  ewma_geral_monte_carlo_resumo %>%
    select(-`(λ;α)`) %>%
    mutate(across(where(is.numeric), \(x) round(x, digits = 4))),
  caption = "Pontos fora de controle por Φ₁ e λ",
  colnames = c("Valores de λ", "Valores de α", "Valores de Φ₁", "Média", "Mínimo", "Máximo")
)
```
...
ggplotly(
  ewma_geral_monte_carlo_resumo %>%
    # get `(λ;α)` where (h1_phi == 0.2 and mean >= 0.05 & mean < 0.06)
    group_by(`(λ;α)`) %>%
    summarise(
      h1_phi = list(h1_phi),
      mean = list(mean),
    ...
    ggplot(aes(x = h1_phi, y = mean, color = `(λ;α)`)) +
    geom_line() +
    geom_point() +
    labs(
      x = "Valores de Φ",
      y = "Fração de pontos fora de controle",
      color = "Valores de (λ;α)",
      title = "FPFCs por Φ, λ e α"
    ) +
    theme.base
) %>%
  plotly.base
...
```{r warning=FALSE}
ggplotly(
  ewma_geral_monte_carlo %>%
    ggplot(aes(x = factor(h1_phi), y = fracao_fora_de_controle, fill = `(λ;α)`)) +
    geom_boxplot() +
    labs(
      x = "Valores de Φ",
      y = "Fração de pontos fora de controle",
...
Observando que ao aumentar o valor de $\alpha$, mantendo $\lambda$ fixo, há um leve aumento na variabilidade da fração de pontos fora de controle. E, ao aumentar o valor de $\lambda$, mantendo $\alpha$ fixo, há uma redução significativa dessa variabilidade.

## Comparando os resultados

Nota-se que houve um aumento de poder considerável.

```{r}
estatisticas_ewma_resumo_combinadas <- bind_rows(
  ewma_geral_monte_carlo_resumo %>%
    filter(`(λ;α)` == "0.3;0.02") %>%
    rename(parametro = `(λ;α)`) %>% mutate(algoritmo = "EWMA(λ,α)"),
  ewma_monte_carlo_resumo %>%
    filter(lambda == 0.7) %>%
...
```{r}
ggplotly(
  estatisticas_ewma_resumo_combinadas %>%
    ggplot() +
    geom_line(
...
ewma_ar <- function(dados, lambda, x0 = NULL, desvio = NULL) {
...
ewmar_lambda_otimo <- cache_dados(
  "ewmaar-lambda-otimo",
  \() {
    data.frame(
      id = 1:100
...
```{r}
ewmaar_monte_carlo <- cache_dados(
  "simulacao-ewma-ar",
  function() {
    gerador_monte_carlo(
      parametros = list(
      ...
      mutate(
        ewma_ar = list(ewma_ar(h1_controle, lambda, h0_amostras[nH0], sd(h0_amostras))),
...
ggplotly(
  ewmaar_monte_carlo %>%
    ...
    unnest(
      cols = c(`Φ₁`, observacao, h1_controle, LI, LS, fora_de_controle)
    ) %>%
    ggplot() +
    geom_line(aes(x = observacao, y = LI, color = "Limite Inferior"), linetype = "dashed") +
    geom_line(aes(x = observacao, y = LS, color = "Limite Superior"), linetype = "dashed") +
    geom_line(aes(x = observacao, y = h1_controle, color = "Série")) +
    geom_point(data = . %>% filter(fora_de_controle),
               aes(x = observacao, y = h1_controle, color = "Fora de controle"), size = 1, shape = 4) +
    labs(x = "Observação", y = "Caracterísitca", color = "Legenda") +
    scale_color_manual(
      values = c(
        "Limite Inferior" = adjustcolor("red", alpha.f = 0.5),
    ...
    labs(
      title = "EWMA para resíduos βAR(1), com λ = 0.08",
    ) +
    theme.base +
    theme(
      legend.position = "bottom",
...
ggplotly(
  cusum_para_k_otimo %>%
    ...
    geom_point(data = . %>% filter(fora_de_controle_pos),
               aes(x = observacao, y = pos, color = "Fora de controle"), size = 1, shape = 4) +
    geom_point(data = . %>% filter(fora_de_controle_neg),
               aes(x = observacao, y = neg, color = "Fora de controle"), size = 1, shape = 4) +
    labs(x = "Observação", y = "Caracterísitca", color = "Legenda") +
    scale_color_manual(
      values = c(
...
estatisticas_resumo_combinadas <- bind_rows(
  ...
  comb_cusum_resumo %>%
    ...
    mutate(
      `(K,H)` = as.factor(paste(intervalo_de_decisao, desvio_detectavel, sep = ";")),
      algoritmo = "CUSUM(K,H)"
    ) %>%
...
comb_monte_carlo <- cache_dados(
  "simulacao-ewma-cusum",
  function() {
    gerador_monte_carlo(
      parametros = expand.grid(
        desvio_detectavel = seq(0.6, 1.2, by = 0.2),
        intervalo_de_decisao = seq(2, 5, by = 1),
        lambda = seq(0.2, 1.0, by = 0.2),
        alfa = seq(0.03, 0.1, by = 0.03)
      ),
      numero_de_execucoes = 50
    ) %>%
      mutate(
        "(K;H;λ;α)" = factor(paste(desvio_detectavel, intervalo_de_decisao, lambda, alfa, sep = ";")),
        ewma = list(ewma_qcc(h0_residuos, lambda, h1_residuos, qnorm(1 - alfa))),
...
comb_coop_monte_carlo_resumo <- comb_monte_carlo %>%
  group_by(`(K;H;λ;α)`, h1_phi, desvio_detectavel, lambda, alfa) %>%
  summarise(
...
datatable(
  comb_coop_monte_carlo_resumo %>%
    select(-`(K;H;λ;α)`) %>%
    mutate(across(where(is.numeric), \(x) round(x, digits = 4))),
  caption = "Pontos fora de controle por Φ₁ e (K;H;λ;α)",
  colnames = c("Valores de Φ₁", "K", "H", "λ", "α", "Média", "Mínimo", "Máximo")
)
```
...
ggplotly(
  comb_coop_monte_carlo_resumo %>%
    # get `(K;H;λ;α)` where h1_phi == 0.2 and mean >= 0.05 & mean < 0.06
    group_by(`(K;H;λ;α)`) %>%
    summarise(
      h1_phi = list(h1_phi),
      mean = list(mean),
    ...
    filter(map_lgl(mean, ~ .x[1] < 0.08)) %>%
    unnest(cols = c(h1_phi, mean)) %>%
    ggplot(aes(x = h1_phi, y = mean, color = `(K;H;λ;α)`)) +
    geom_line() +
    geom_point() +
    labs(
      x = "Valores de Φ₁",
      y = "Fração de pontos fora de controle",
      color = "Constantes (K;H;λ;α)",
      title = "FPFC por Φ₁ e (K;H;λ;α)"
    ) +
    theme.base
) %>%
  plotly.base
...
estatisticas_combinadas_resumo_ew_sum <- bind_rows(
  estatisticas_resumo_combinadas,
  comb_coop_monte_carlo_resumo %>%
    filter(`(K;H;λ;α)` == "1;5;0.6;0.03") %>%
    mutate(parametro = as.factor(`(K;H;λ;α)`)) %>%
    mutate(algoritmo = "EW-SUM(coop)") %>%
    select(algoritmo, parametro, h1_phi, mean),
  comb_opos_monte_carlo_resumo %>%
    filter(`(K;H;λ;α)` == "0.6;5;0.4;0.09") %>%
    mutate(parametro = as.factor(`(K;H;λ;α)`)) %>%
