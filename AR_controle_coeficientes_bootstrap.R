...
passo <- 30 # Passo para a sequência de colagens
bc <- c(1) # c(1, 3, 5, 7, 9)
...
p1 <- ggplot(DADOS_RESUMO, aes(x = x_label, y = proporcao, color = factor(desvio))) +
  ...
  geom_text(aes(x = min(x_label) - 5, y = 0.05, label = "0,05"), vjust = 0, hjust = 0.5, size = 4, show.legend = FALSE) +
  geom_errorbar(aes(ymin = proporcao - dp, ymax = proporcao + dp), width = 1, alpha = 0.5) +
  labs(
